...
flowchart TD
    ...
    G --> A
    F -->|예| H["모든 유효 게임의 6개 요소 응답"]
    H --> I{"모든 요소에 응답했는가?"}
    I -->|아니요| J["누락 항목 안내"]
    J --> H
    I -->|예| K["입력 저장"]
    K --> L["요소별 맞을 근거·주의 신호·판단 불가 확인"]
    L --> M["기본 3개 리뷰 근거 확인"]
    M --> N{"더 볼 것인가?"}
    N -->|예| O["최대 10개까지 펼치기"]
    N -->|아니요| P["구매 판단 저장"]
...
3. 구매를 고민하는 게임을 선택한다.
4. 전투, 조작, 보스전, 탐험, 성장·빌드, 파밍·반복 플레이별 기본 리뷰 분석을 확인한다.
...
2. 각 게임에서 다음 6개 요소에 모두 응답한다.
   ...
   - 성장·빌드
   - 파밍·반복 플레이
3. 각 요소에서 `좋았음`, `아쉬웠음`, `경험 부족` 중 하나를 선택한다.
4. 진행률과 아직 응답하지 않은 항목을 확인한다.
5. 모든 요소에 응답하면 저장할 수 있다.

`경험 부족`은 응답 완료에는 포함하지만 해당 게임·요소의 취향 계산에는 사용하지 않는다. 일부 요소가 `경험 부족`이어도 게임별 플레이 조건을 충족했다면 최소 3개 게임 수에는 포함한다.

작성 중인 입력은 `localStorage`에 자동 저장한다. 사용자가 페이지를 새로고침하거나 다시 방문하면 초안을 복구한다. 저장 버튼을 누르면 완료 상태를 로컬에 기록하며 서버로 전송하지 않는다.

## 5. 개인화 계산 범위

- 조건을 충족하고 6개 요소 응답을 완료한 모든 비교 기준 게임을 사용한다.
- 사용자가 계산에 사용할 3개 게임만 따로 선택하지 않는다.
- 각 요소에서 `좋았음` 또는 `아쉬웠음`으로 응답한 게임만 그 요소의 분모에 포함한다.
- `경험 부족`과 특성 미발견은 부정 신호로 해석하지 않는다.
- 특성의 분포 범위와 사용자의 선호 일관성을 별도로 계산한다.
- 한 게임에서만 확인된 특성은 반복 취향이 아니라 잠정적인 근거로 취급한다.

## 6. 개인화 결과 확인

1. 사용자가 분석 대상 게임을 선택한다.
2. 시스템이 저장된 사용자 평가와 후보 게임의 검수된 리뷰 특성을 비교한다.
...
4. 한 요소에 맞을 근거와 주의 신호가 모두 있으면 함께 보여준다.
...
3. 비교 기준 게임의 6개 요소 평가를 확인하고 수정한다.
...
| 6개 요소 중 응답하지 않은 항목이 있음 | 저장을 막고 누락 항목과 진행률 안내 |
| 일부 요소를 `경험 부족`으로 응답함 | 입력 완료로 인정하고 해당 요소 계산에서만 제외 |
| 같은 요소에 긍정·주의 근거가 모두 존재함 | 한쪽을 제거하지 않고 함께 표시 |
| 비교 특성이나 리뷰 근거가 부족하거나 충돌함 | 해당 요소를 `판단 불가`로 표시 |
